...
const Modal = ({ children, isOpen, onClose, title }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex justify-center items-center z-50 p-4">
            <div className="bg-gray-800 rounded-2xl shadow-2xl w-full max-w-3xl border border-gray-700">
    ...
    return (
        <div className="flex items-center" onMouseLeave={() => setHoverScore(0)} onClick={(e) => e.stopPropagation()}>
            {[...Array(5)].map((_, i) => {
                ...
                return (<Star key={i} size={18} className={`cursor-pointer transition-colors ${(hoverScore || score) >= currentScore ? 'text-yellow-400 fill-current' : 'text-gray-600'}`} onMouseEnter={() => setHoverScore(currentScore)} onClick={() => handleClick(currentScore)}/>);
            })}
        </div>
    );
};
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end pt-4 gap-3">
                ...
                <button type="submit" disabled={isSaving} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-blue-400">{isSaving ? 'Salvando...' : 'Salvar Projeto'}</button>
            </div>
        </form>
    );
};
...
const CsvImportModal = ({ onClose }) => {
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            await batch.commit();
            alert(`${investorsToImport.length} investidores importados com sucesso!`);
            onClose();
        } catch (error) { console.error("Erro ao importar CSV:", error); alert("Ocorreu um erro na importação."); } 
    ...
    return (
        <>
            ...
            <div className="flex justify-end pt-4 gap-3">
                ...
                <button onClick={handleImport} disabled={isImporting} className="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 disabled:bg-green-400">{isImporting ? 'Importando...' : 'Importar'}</button>
            </div>
        </>
    );
};
...
const AddInvestorToProjectModal = ({ onClose, selectedProjectId }) => {
    const { userId, masterInvestors, projectInvestors } = useContext(DataContext);
    ...
    const availableInvestors = useMemo(() => {
        ...
        return masterInvestors.filter(mi => !projectInvestorIds.has(mi.id) && (classificationFilter === '' || mi.classificacao === classificationFilter) && (sectorFilter === '' || mi.setor === sectorFilter));
    ...
    const handleAdd = async () => {
        ...
        try {
            ...
            await batch.commit();
            onClose();
        } catch (error) { console.error("Erro ao adicionar investidores:", error); alert("Falha ao adicionar investidores."); }
    ...
    return (
        <>
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
                {availableInvestors.map(investor => (
                    <div key={investor.id} onClick={() => toggleSelection(investor.id)} className={`flex items-center gap-3 p-3 rounded-md cursor-pointer transition-colors ${selectedInvestors.includes(investor.id) ? 'bg-blue-900 border border-blue-600' : 'bg-gray-700 hover:bg-gray-600'}`}>
                        <div className={`w-5 h-5 rounded-sm border-2 flex items-center justify-center ${selectedInvestors.includes(investor.id) ? 'bg-blue-600 border-blue-500' : 'border-gray-500'}`}>{selectedInvestors.includes(investor.id) && <CheckCircle size={16} className="text-white" />}</div>
                        <div><p className="font-medium text-white">{investor.nomeFantasia}</p><p className="text-sm text-gray-400">{investor.setor}</p></div>
                    </div>
                ))}
            ...
            <div className="flex justify-end pt-4 gap-3">
                <button type="button" onClick={onClose} className="px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700">Cancelar</button>
                <button onClick={handleAdd} disabled={isAdding || selectedInvestors.length === 0} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-blue-400">{isAdding ? 'Adicionando...' : `Adicionar ${selectedInvestors.length}`}</button>
            </div>
        </>
    );
};
...
const MasterInvestorList = ({ onSelectInvestor }) => {
    ...
    const handleDelete = async (e, investorId, investorName) => {
        e.stopPropagation();
        if (window.confirm(`Tem certeza que deseja excluir ${investorName} da base central? Esta ação é irreversível.`)) {
            try { await deleteDoc(doc(db, 'artifacts', appId, 'users', userId, 'investors', investorId)); alert(`${investorName} foi excluído.`); }
            catch (error) { console.error("Erro ao excluir investidor:", error); alert("Falha ao excluir."); }
        }
    };
    return (
        <div className="p-4 sm:p-6 lg:p-8">
            ...
            <div className="bg-gray-800 rounded-lg shadow-xl p-6">
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-4">
                    <div className="lg:col-span-2"><InputField placeholder="Buscar por nome, setor..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>
                        ...
                        <tbody className="divide-y divide-gray-700">
                           {filteredInvestors.map(investor => (
                                <tr key={investor.id} onClick={() => onSelectInvestor(investor, true)} className="hover:bg-gray-700 cursor-pointer">
                                    ...
                                    <td className="p-4"><div className="flex items-center">{[...Array(5)].map((_, i) => <Star key={i} size={18} className={i < (investor.nota || 0) ? 'text-yellow-400 fill-current' : 'text-gray-600'} />)}</div></td>
                                    <td className="p-4"><button onClick={(e) => handleDelete(e, investor.id, investor.nomeFantasia)} className="text-gray-400 hover:text-red-500 p-2 rounded-full transition-colors"><Trash2 size={18} /></button></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
...
const Dashboard = ({ onSelectInvestor }) => {
    ...
    if (!selectedProjectId) return <div className="flex flex-col items-center justify-center h-full text-center p-8"><Folder size={64} className="text-gray-600 mb-4" /><h2 className="text-2xl font-bold text-white">Nenhum projeto selecionado</h2><p className="text-gray-400 mt-2">Crie ou selecione um projeto.</p></div>
    return (
        <div className="p-4 sm:p-6 lg:p-8">
            <div className="bg-gray-800 rounded-lg shadow-xl p-6">
                <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-4">
                 ...
                 <div className="overflow-x-auto">
                    <table className="w-full text-left text-gray-300">
                     ...
                     {sortedAndFilteredInvestors.length === 0 && <div className="text-center py-10 text-gray-500"><p>Nenhum investidor neste projeto.</p><p>Clique em "Adicionar" para incluir.</p></div>}
                 </div>
            </div>
...
const KanbanBoard = ({ onSelectInvestor }) => {
    ...
    if (!selectedProjectId) return <div className="flex flex-col items-center justify-center h-full text-center p-8"><Folder size={64} className="text-gray-600 mb-4" /><h2 className="text-2xl font-bold text-white">Nenhum projeto selecionado</h2><p className="text-gray-400 mt-2">Selecione um projeto.</p></div>
    return (
...
const InvestorProfile = ({ investor, onBack, isMasterProfile }) => {
    const { userId, selectedProjectId } = useContext(DataContext);
    const [interactionType, setInteractionType] = useState('Email');
    const [interactionNotes, setInteractionNotes] = useState('');
    const handleAddInteraction = async (e) => {
        e.preventDefault(); if (!interactionNotes.trim() || !userId || !selectedProjectId || isMasterProfile) return;
        const newInteraction = { data: new Date(), tipo: interactionType, anotacoes: interactionNotes };
        try {
            await updateDoc(doc(db, 'artifacts', appId, 'users', userId, 'projects', selectedProjectId, 'pipeline', investor.id), { historicoDeInteracoes: [...(investor.historicoDeInteracoes || []), newInteraction] });
            setInteractionNotes('');
        } catch (error) { console.error("Erro ao adicionar interação:", error); }
    };
    return (
        <div className="p-4 sm:p-6 lg:p-8">
            ...
            <div className="bg-gray-800 rounded-lg shadow-xl p-6 mb-6">
                 <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                    <div>
                        ...
                        <div className="flex items-center mt-2"><p className="text-gray-400 mr-2">Nota Geral:</p>{[...Array(5)].map((_, i) => <Star key={i} size={22} className={i < (investor.nota || 0) ? 'text-yellow-400 fill-current' : 'text-gray-600'} />)}</div>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-6">
                             ...
                             <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
                               {investor.historicoDeInteracoes && [...investor.historicoDeInteracoes].reverse().map((item, index) => (
                                   <div key={index} className="bg-gray-700 p-4 rounded-md">
                                        <div className="flex justify-between items-center text-sm mb-1"><p className="font-semibold text-blue-400">{item.tipo}</p><p className="text-gray-400">{new Date(item.data.toDate()).toLocaleString('pt-BR')}</p></div><p className="text-gray-300">{item.anotacoes}</p>
                                    </div>
                               ))}
                               {(!investor.historicoDeInteracoes || investor.historicoDeInteracoes.length === 0) && <p className="text-gray-500 text-center py-4">Nenhuma interação registrada.</p>}
                            </div>
                        </div>
                        </>
                    )}
                 ...
                 <div className="bg-gray-800 rounded-lg shadow-xl p-6 self-start">
                    <h3 className="text-xl font-bold text-white mb-4">Contatos</h3>
...
export default function App() {
    ...
    useEffect(() => {
        if (!userId) {
            setProjects([]); setMasterInvestors([]);
            return;
        }
        setIsLoading(true);
        ...
        const unsubProjects = onSnapshot(projectsQuery, snap => {
            ...
            else if (data.length === 0) setSelectedProjectId(null);
            setIsLoading(false);
        });
        const investorsQuery = query(collection(db, 'artifacts', appId, 'users', userId, 'investors'));
        const unsubInvestors = onSnapshot(investorsQuery, snap => setMasterInvestors(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
        return () => { unsubProjects(); unsubInvestors(); };
    }, [userId, selectedProjectId]);

    useEffect(() => {
        if (!selectedProjectId || !userId) { setProjectInvestors([]); return; };
        const pipelineQuery = query(collection(db, 'artifacts', appId, 'users', userId, 'projects', selectedProjectId, 'pipeline'));
        const unsubPipeline = onSnapshot(pipelineQuery, async (pipelineSnap) => {
    ...
    return (
        <DataContext.Provider value={{ userId, projects, selectedProjectId, setSelectedProjectId, masterInvestors, projectInvestors, setIsProjectModalOpen, setIsCsvModalOpen }}>
            <div className="min-h-screen bg-gray-900 text-white font-sans flex flex-col sm:flex-row">
                 <nav className="bg-gray-800 p-4 flex sm:flex-col items-center gap-4 border-b sm:border-b-0 sm:border-r border-gray-700 w-full sm:w-64 shrink-0">
                     ...
                     <div className="flex sm:flex-col items-center gap-2 w-full mt-0 sm:mt-4">
                        <button onClick={() => { setCurrentPage('master_list'); setSelectedInvestor(null); }} className={`p-3 rounded-lg flex items-center gap-3 w-full ${currentPage === 'master_list' ? 'bg-blue-600' : 'hover:bg-gray-700'}`}><Database size={20} /><span className="hidden sm:inline">Base de Investidores</span></button>
                        ...
                        <button onClick={() => { setCurrentPage('dashboard'); setSelectedInvestor(null); }} className={`p-3 rounded-lg flex items-center gap-3 w-full ${currentPage === 'dashboard' ? 'bg-blue-600' : 'hover:bg-gray-700'}`}><Users size={20} /><span className="hidden sm:inline">Dashboard</span></button>
                        <button onClick={() => { setCurrentPage('kanban'); setSelectedInvestor(null); }} className={`p-3 rounded-lg flex items-center gap-3 w-full ${currentPage === 'kanban' ? 'bg-blue-600' : 'hover:bg-gray-700'}`}><Kanban size={20} /><span className="hidden sm:inline">Pipeline</span></button>
            ...
            <Modal isOpen={isProjectModalOpen} onClose={() => setIsProjectModalOpen(false)} title="Novo Projeto"><ProjectForm onClose={() => setIsProjectModalOpen(false)}/></Modal>
